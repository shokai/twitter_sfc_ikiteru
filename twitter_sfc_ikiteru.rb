...
tweet = nil
if per >= 1
  tweet = ['ばりばりです', "バリバリです", '元気です', 'ikiteru', '平穏無事', 'ok', 'すごくいい',
           'めでたい', 'ド'*rand(10), 'いい感じ', '完璧', 'ばっちりです', 'かず助いきたい',
           'ぱっちりです'].choice
elsif per <= 0.3
  tweet = ["ダメ", "駄目", "ikitenai", "完全に沈黙", "オワコン", "http://このまま眠り続けて.死ぬ.jp", 
           "寝てます", "もう寝よう！", "(´・ω・｀)", "＼(^o^)／ｵﾜﾀ", '無理', 'もう寝ろ', 'ぐぬぬ',
          'こんなのって・・・ないよ・・・', '狂気〜〜ッ'].choice
else
  tweet = ["微妙", "動いている、ような" , "ちょっと動いてる", "ところどころ動いてる", 
           "ねえ、今動いたわ！", 'うーん', '要注意です', 'まあいいんじゃないの', 
           'だいたい動いてる', '合格', 'そこそこ', '動いてないサーバーもあるし、動いてるのもあるし',
           'gem install sfc_ikiteru'].choice
end
